...
void *server_bootable() {
  ...
  if (pushlock == LOCKED) {
    ...
    if ((last_heartbeat() < HEARTBEAT_TIMEOUT || detect_heartbeat() > 0 )) { // && Serial
      serial_flush();
...
void *server_running() {
  ...
  if (pushlock == LOCKED) {
    if (last_heartbeat() > 30000 || detect_heartbeat() < 0 ) {
...
void *server_hungup() {
  ...
  heartbeart = detect_heartbeat();
  heartbeart = last_heartbeat();

  if (heartbeart < HEARTBEAT_TIMEOUT || detect_heartbeat() > 0) {

    return server_running;
  } else {

    if (heartbeart > HEARTBEAT_TIMEOUT && heartbeart < 20000UL) {
      fade_hungup(25);
    } else if (heartbeart > 20000UL && heartbeart < 30000UL) {
      fade_hungup(15);
    } else if (heartbeart > 30000UL && heartbeart < 450000UL) {
      fade_hungup(5);
    }

    if (heartbeart > 45000UL) {
      show_red();
    }
